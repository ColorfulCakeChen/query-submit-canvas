...
//     this.offspringChromosomes = new Array( entityChromosomeCount );
   ...
   *   Every element of base64EncodedStringArray is a string whose content is
   * base64 encoded text.
   *
   ...
   *   Return this when every time yield. Usually, this is the root container of
   * the progressParent.
   *
   * @param {ValueMax.Percentage.Aggregate} progressParent
   *   This should be progressRoot or some descendant of progressRoot. A new
   * progressToAdvance will be created and added to progressParent. The
   * progressToAdvance will be increased when every time advanced.
   *
   ...
   */
  static *decoder_Base64_StringArray_To_Uint8Array(
    base64EncodedStringArray, textEncoder, progressRoot, progressParent ) {
...
/**
 * Download differential evolution individual weights (entity vs entity) from
 * network (a publish html of Google Sheets).
 *
 ...
 *   - The ExportSheetNo is the array index of the Google Sheet. This is mainly used
 *       by server. Client (downloader) can ignore this value.
 ...
 *     - A versus id contains EntityNo, ParentGenerationNo, OffspringGenerationNo
 *         which are separated by underline (_).
...
//!!! ...unfinished... change to async generator.
   ...
   *   The published (should be html, not tsv, not csv) web page URL of the versus
   ...
   * The "gid=474257400" inside the URL should be the same as the 1st line of the
   * table of the web page.
   *
   * @param {ValueMax.Percentage.Aggregate} progressToYield
   *   Return this when every time yield. Usually, this is the root container of the
   * progressToAdvance.
   *
   * @param {ValueMax.Percentage.Concrete} progressToAdvance
   *   Increase this when every time progress advanced. It will be initialized to
   * zero when decoder starting.
   *
  ...
  async *fetcher( versusURL, progressToYield, progressToAdvance ) {
    ...
    this.gid_versus.set_ByString( lineMatch.value[ 1 ], gidString );

    // 4. Parse every gid and their corresponding weights (of chromosomes of parent
    // and offspring).
    this.enitiyChromosomesArray
      = new Array( this.gid_versus.VersusId_WinCount_Array.length );

    // All chromosomes of one entity's parent (or offspring).
    let chromosomeArray = new Array( this.entityChromosomeCount );

    // All parent's chromosomes of all entities.
    for ( let i = 0;
          ( i < this.enitiyChromosomesArray.length ) && ( !lineMatch.done );
          ++i, ( lineMatch = lineMatch.next() ) ) {
      let entityNo = this.gid_versus.VersusId_WinCount_Array[ i ].entityNo;
      let enitiyChromosomes = this.enitiyChromosomesArray[ i ]
        = new EnitiyChromosomes( entityNo, this.entityChromosomeCount );

      // The parent's chromosomes of the entity.
      {
        let k;
        for ( k = 0;
              ( k < this.entityChromosomeCount ) && ( !lineMatch.done );
              ++k, ( lineMatch = lineMatch.next() ) ) {
          chromosomeArray[ k ] = lineMatch.value[ 1 ];
        }
        chromosomeArray.fill( null, k ); // For not enough chromosome, keep them null.
        enitiyChromosomes.parentChromosome
          = EnitiyChromosomes.decode_Base64_StringArray_To_Uint8Array(
              chromosomeArray, this.textEncoder );
      }
    }
    ...
    for ( let i = 0;
      ...
      // The offspring's chromosomes of the entity.
      {
        ...
        chromosomeArray.fill( null, k ); // For not enough chromosome, keep them null.
        enitiyChromosomes.offspringChromosome
          = EnitiyChromosomes.decode_Base64_StringArray_To_Uint8Array(
              chromosomeArray, this.textEncoder );
      }
    }
