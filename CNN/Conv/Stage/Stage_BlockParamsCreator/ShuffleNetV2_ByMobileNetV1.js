 ...
 * 1. Motivation
 *
 * Accodring to testing, the original ShuffleNetV2 is faster than MobileNetV2
 * in backend CPU. This may result from lesser computation. However, in backend
 * WASM and WEBGL, MobileNetV2 is faster than the original ShuffleNetV2. The
 * possible reason may be that the concatenate-shuffle-split (even achieved by
 * pointwise convolution) operation is not friendly for WASM and WebGL.
 *
 ...
 *
 * This may become a faster ShuffleNetV2 in backend WASM and WEBGL (but a
 * slower ShuffleNetV2 in backend CPU due to more computation).
 *
 *
...
class ShuffleNetV2_ByMobileNetV1 extends Base {
  ...
  configTo_beforeBlock0() {
    ...
    this.nConvBlockTypeId = ValueDesc.ConvBlockType.Singleton.Ids
      .SHUFFLE_NET_V2_BY_MOBILE_NET_V1_HEAD;

    if ( stageParams.bPointwise1 == false ) {

      // In ShuffleNetV2_ByMobileNetV1's head,
      // if ( stageParams.bPointwise1 == false ), pointwise1ChannelCount is
      // also 0.
      //
      // But, in this case, pointwise1 will still be created by Block
      // intrinsically and output double of input0.
      //
      ...
      // In ShuffleNetV2_ByMobileNetV1's head,
      // if ( stageParams.bPointwise1 == true ), pointwise1ChannelCount is
      // always the same as input0's channel count.
      //
    ...
    // In ShuffleNetV2_ByMobileNetV1, pointwise20ChannelCount is always twice
    // of original input0's channel count.
  ...
  configTo_beforeBlockN_exceptBlock0( blockIndex, input_height, input_width ) {
    ...
    // if ( stageParams.bPointwise1 == false ), pointwise1ChannelCount is also
    // 0. In this case, pointwise1 will not be created by PointDepthPoint (i.e.
    // different from block0).
    //
    if ( stageParams.bPointwise1 == false ) {
      this.pointwise1ChannelCount = 0; // (Intrinsically, zero, too.)

    // In ShuffleNetV2_ByMobileNetV1's body/tail,
    // if ( stageParams.bPointwise1 == true ), pointwise1ChannelCount is always
    // the same as pointwise20 output channel count (which is already doubled
  ...
  configTo_beforeBlockLast() {
    ...
    this.nConvBlockTypeId = ValueDesc.ConvBlockType.Singleton.Ids
      .SHUFFLE_NET_V2_BY_MOBILE_NET_V1_TAIL;
  }
}
