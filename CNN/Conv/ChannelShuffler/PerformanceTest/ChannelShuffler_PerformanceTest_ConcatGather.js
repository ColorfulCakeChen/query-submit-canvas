...
import { ConcatGather as ChannelShuffler_ConcatGather }
  from "../ChannelShuffler_ConcatGather.js";
...
class ConcatGather extends ChannelShuffler_ConcatGather {
  ...
  /**
   * Used as default ChannelShuffler.PerformanceTest.ConcatGather provider for
  ...
  constructor( concatenatedShape, outputGroupCount ) {
    ...
    ConcatGather.setAsConstructor_self.call( this,
  ...
  gather_map( concatenatedTensor ) {
    ...
    return this.shuffledChannelIndicesTensor1dArray.map(
      shuffledChannelIndicesTensor1d =>
        concatenatedTensor.gather(
          shuffledChannelIndicesTensor1d, this.shuffleInfo.lastAxisId )
    );
  }
  ...
  concatGather_dispose_direct_call_loop( tensorArray ) {
    ...
    let shuffledSplitedTensorArray
      = this.gather_loop( concatenatedTensor );
  ...
  concatGather_dispose_finally_map( tensorArray ) {
    ...
    try {
      // shuffle and split by gather (one operation achieves two operations).
      let shuffledSplitedTensorArray
        = this.shuffledChannelIndicesTensor1dArray.map(
            shuffledChannelIndicesTensor1d =>
              concatenatedTensor.gather(
                shuffledChannelIndicesTensor1d,
                this.shuffleInfo.lastAxisId )
